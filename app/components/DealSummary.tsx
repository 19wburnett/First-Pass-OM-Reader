...
export default function DealSummary({ dealData }: DealSummaryProps) {
  ...
  const [targetIRR, setTargetIRR] = useState(0.15) // Default target IRR
    ...
    <div className="space-y-6">
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-center justify-between">
          <div>
            <h6 className="text-sm font-medium text-blue-900 mb-1">Target IRR vs. Actual</h6>
            <p className="text-xs text-blue-700">
              Compare your target return with the projected IRR
            </p>
          </div>
        ...
        <div className="mt-4 flex items-center space-x-4">
          <div className="flex-1">
            ...
            <input
              ...
              max="100"
              value={targetIRR * 100}
              onChange={(e) => setTargetIRR(parseFloat(e.target.value) / 100)}
              className="w-full px-3 py-2 border border-blue-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-blue-900"
              placeholder="15.0"
            />
          </div>
          <div className="text-center">
            <div className={`text-lg font-bold ${
              currentDealData.leveredIRR && currentDealData.leveredIRR >= targetIRR 
                ? 'text-green-600' 
                : 'text-red-600'
            }`}>
              {currentDealData.leveredIRR && currentDealData.leveredIRR >= targetIRR ? '✓' : '✗'}
            ...
            <div className="text-xs text-blue-700">
              vs {targetIRR * 100}% Target
            </div>
          </div>
        </div>
        
        {/* Target Status */}
        <div className="mt-3 p-2 rounded-md bg-white border border-blue-200">
      ...
      {/* User Assumptions Used */}
      <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
        <h6 className="text-sm font-medium text-green-900 mb-3">Assumptions Used in Calculations</h6>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div className="text-center">
            <div className="font-medium text-green-900">
              {formatPercent(assumptions.exitCapRate)}
            </div>
            <div className="text-xs text-green-700">Exit Cap Rate</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-green-900">
              {formatPercent(assumptions.loanToValue)}
            </div>
            <div className="text-xs text-green-700">LTV</div>
          </div>
          <div className="text-center">
            <div className="font-medium text-green-900">
        ...
        <p className="text-xs text-green-700 mt-2 text-center">
          These parameters were used for underwriting calculations and IRR analysis
        </p>
      </div>
